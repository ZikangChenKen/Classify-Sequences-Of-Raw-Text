# -*- coding: utf-8 -*-
"""COMP330_A6_TASK4_V2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1STTk1usgbOQQyxHvScPxFA2MDIEWqOZ9
"""
...
hiddenUnits = 500  # change to 500 due to time warping but change to 1000 could improve performance a lot
...
def generateDataRNN (maxSeqLen, data):
    ...
    myInts = np.random.choice(list(data.keys()), batchSize, replace=False)
    #
...
filter = tf.Variable(np.random.normal(0, 0.01, (2560, 10)), dtype=tf.float32)

# the weight matrix that maps the inputs and hidden state to a set of values
Wfir = tf.Variable(np.random.normal(0, 0.01, (hiddenUnits + 10 + hiddenUnits, hiddenUnits)), dtype=tf.float32)
...
state_record = []
tenth_state = initialState
for idx in range(len(sequenceOfLetters) - 9):
    #
    # concatenate the state with the input, then compute the next state
    temp = sequenceOfLetters[idx]
    for i in range(9):
      temp = tf.concat([temp, sequenceOfLetters[idx + i]], 1)  # concatenate 10 characters
    temp = tf.matmul(temp, filter)  # apply the filter
    if len(state_record) == 10:
      tenth_state = state_record.pop(0)
    inputPlusState = tf.concat([temp, currentState, tenth_state], 1)
...
with tf.Session() as sess:
    ...
    for i in range(30):
        ...
        x, y = generateDataRNN(maxSeqLen, test_sub)
        _currentState = np.zeros((batchSize, hiddenUnits))
