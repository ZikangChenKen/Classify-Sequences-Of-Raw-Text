# -*- coding: utf-8 -*-
"""COMP330_A6_Task1.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1fNda-aQHjqwr84mHIAvxOXZ0jIEC-oTY
"""
...
with tf.Session() as sess:
    ...
    for i in range(0, len(test), batchSize):
        # stack all of the text into a matrix of one-hot characters
        x = np.stack([data[i][1] for i in range(i, i + batchSize)])
        #
        # and stack all of the labels into a vector of labels
        y = np.stack([np.array((data[i][0])) for i in range(i, i + batchSize)])
        _currentState = np.zeros((batchSize, hiddenUnits))
    ...
    print("Loss for {size} randomly chosen documents is {loss}, number correct labels is {numCorrect} out of {size}".format(size=len(test), loss=loss, numCorrect=numCorrect))